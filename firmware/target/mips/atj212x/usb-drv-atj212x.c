...
#if ATJ212X_MCLK_SEL != ATJ212X_CLK_PLLB
static inline void usb_enable_pll(void)
{
    CGU_COUNTB = CGU_LOCK_CNT;
    CGU_PLLB = ATJ212X_PLLB_SETTING;
...
static void usb_phy_on(void)
{
    /* PHY clock */
    CGU_USB = 1<<5 /* enable */
        | (CLK_DIV(ATJ212X_PLLB_FREQ, 48000000) / 2) << 2
        | 2; /* source = PLLB */

    /* UVDD on */
    ascodec_write(ATJ212X_USB_UTIL, ascodec_read(ATJ212X_USB_UTIL) | (1<<4));
    mdelay(100);
